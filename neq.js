...
/////
const app = express()
app.get('/', (req, res) => res.send("Bot Aktif | Neq"))
...
client.on("ready", () => {
  console.log(`Bütün komutlar başarıyla yüklendi Amünyum!`);
  client.user.setStatus("Do Not Disturb");
  client.user.setActivity('Neq <3 | #S3X');
})
...
fs.readdir('./komutlar/', (err, files) => {
  ...
  log(`${files.length} adet komut yüklemeye hazırlanılıyor amünyum.`);
  files.forEach(f => {
    let props = require(`./komutlar/${f}`);
    log(`Yüklenen komut ismi bu amünyum: ${props.help.name.toUpperCase()}.`);
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea'|| msg.content.toLowerCase() == 'selam') {
          try {
                  ...
                  return msg.reply(
                    'Aleyküm Selam Babba, Hoş Geldin')
          } catch(err) {
            console.log(err);
          }
      }
